...
// skeleton
const SkeletonCard = () => (
  <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 p-5 flex gap-4 animate-pulse">
...
const EmptyState = ({ tab, hasFilters }) => (
  <div className="flex flex-col items-center justify-center py-20 text-slate-400 dark:text-slate-500">
    <div className="w-16 h-16 rounded-2xl bg-blue-50 dark:bg-blue-900/30 flex items-center justify-center mb-4 text-blue-500 dark:text-blue-400">
    ...
    <div className="flex items-center justify-between mt-6">
      ...
      <div className="flex items-center gap-1.5">
        <button
          ...
          className="w-8 h-8 flex items-center justify-center rounded-lg border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-800 text-slate-500 dark:text-slate-400
                     hover:bg-blue-50 dark:hover:bg-slate-700 hover:border-blue-300 hover:text-[#0d1b4b]
        ...
        {pages.map((p, idx) =>
          p === "…" ? (
            <span key={`e-${idx}`} className="w-8 h-8 flex items-center justify-center text-slate-400 dark:text-slate-500 text-sm select-none">
              …
            </span>
          ) : (
            <button
...
const FilterPills = ({ activeFilters, onToggle, onClear }) => (
  <div className="flex flex-wrap items-center gap-2 mb-4">
    <span className="text-[11px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest mr-1">Filter</span>
        ...
        <button
          ...
          className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-semibold border transition-all duration-150 cursor-pointer
            ${isActive
              ...
              : "bg-white dark:bg-slate-800 text-slate-500 dark:text-slate-400 border-slate-200 dark:border-slate-600 hover:border-[#0d1b4b] hover:text-[#0d1b4b] dark:hover:border-blue-400 dark:hover:text-blue-300"
            }`}
        >
          {label}
    ...
    {activeFilters.length > 0 && (
      <button
        onClick={onClear}
        className="text-[11px] font-semibold text-slate-400 dark:text-slate-500 hover:text-red-500 dark:hover:text-red-400 transition-colors cursor-pointer underline underline-offset-2 ml-1"
      >
        Clear all
      </button>
    )}
...
const ReservationCard = ({ resv, onCancelRequest, cancelling, onHide }) => {
  ...
  return (
    <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 p-5 flex gap-4 items-start shadow-sm hover:shadow-md hover:-translate-y-0.5 transition-all duration-200">

      {/* Book cover */}
      <div className="w-[52px] h-[68px] rounded-lg overflow-hidden shrink-0 bg-blue-50 dark:bg-slate-700 flex items-center justify-center text-blue-500 dark:text-blue-300">
        {resv.bookId?.img ? (
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-start justify-between gap-3 flex-wrap">
          <div>
            ...
            <h3 className="mt-2 mb-0.5 text-[15px] font-bold text-[#0d1b4b] dark:text-white leading-snug">
              {resv.bookId?.name || "Unknown Book"}
            </h3>
            <p className="text-[13px] text-slate-500 dark:text-slate-400">By {resv.bookId?.author || "Unknown Author"}</p>
          </div>
          ...
          {canCancel && (
            <button
              ...
              className="shrink-0 px-4 py-1.5 rounded-lg border border-red-400 dark:border-red-500 text-red-500 dark:text-red-400 text-xs font-semibold
                         bg-transparent hover:bg-red-50 dark:hover:bg-red-900/20 disabled:opacity-50 disabled:cursor-not-allowed
        ...
        <div className="flex flex-wrap gap-x-5 gap-y-2 mt-3">
          ...
          {resv.timeRemaining && resv.status === "reserved" && (
            <span className="flex items-center gap-1 text-xs font-semibold text-blue-600 dark:text-blue-400 bg-blue-100 dark:bg-blue-900/40 px-2 py-0.5 rounded-full">
              <ClockIcon />
              {resv.timeRemaining}
            </span>
          )}
        ...
        {resv.isActionRequired && (
          <div className="mt-3 px-3.5 py-2 rounded-lg bg-blue-50 dark:bg-blue-900/30 border-l-4 border-blue-500 text-xs text-[#1a2d6d] dark:text-blue-300 font-medium">
  ...
  return (
    <div className="min-h-screen bg-blue-50 dark:bg-slate-900">
            ...
            {["active", "history"].map((t) => (
              <button
                ...
                className={`px-6 py-2.5 rounded-t-xl text-sm font-semibold capitalize transition-all duration-200 border-none cursor-pointer
                  ${tab === t
                    ? "bg-blue-50 dark:bg-slate-900 text-[#0d1b4b] dark:text-white"
      ...
      <div className="max-w-3xl mx-auto px-6 py-7 pb-16">
        ...
        {/* Error */}
        {error && (
          <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-xl px-4 py-4 flex items-start gap-3">
            ...
            <button
              ...
              className="flex items-center gap-1.5 text-xs font-semibold text-red-500 dark:text-red-400 hover:text-red-700 dark:hover:text-red-300 transition-colors cursor-pointer shrink-0"
            >
            ...
            {!isEmpty && (
              <div className="flex flex-col gap-3">
                ...
                {actionRequired.length > 0 && filteredRest.length > 0 && tab === "active" && (
                  <p className="text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest mt-2 mb-0.5">
                    Other Reservations
                  </p>
                )}
