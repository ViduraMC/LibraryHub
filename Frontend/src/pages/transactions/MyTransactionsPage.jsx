...
// Shows a student or teacher their own borrow history.
// They can renew an active book once for +2 days.
const MyTransactionsPage = () => {
    ...
    const fetchTransactions = async () => {
        try {
            ...
            toast.error('Could not load your borrow history.');
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleRenew = async (id) => {
        ...
        try {
            ...
            // Update the single record in state instead of refetching everything
            setTransactions((prev) =>
                prev.map((t) => (t._id === id ? { ...t, ...res.data.transaction } : t))
            );
        } catch (err) {
    ...
    // How many books are currently active or overdue
                    ...
                    {transactions.map((t) => (
                        ...
                        >
                            ...
                            <div className="flex justify-between items-start mb-6">
                                <span className={`px-3 py-1 text-[10px] font-black uppercase tracking-widest rounded-full border ${statusStyle(t.status)}`}>
                                    {t.status}
                                </span>
                                {t.renewed && (
